...
def normalizar_coordenadas(coordenadas,trp_size):
    if(trp_size >= 500):
        return [[coord[0] / 500, coord[1] / 500] for coord in coordenadas]
    return [[coord[0] / 100, coord[1] / 100] for coord in coordenadas]

def generate_trp_data(dataset_size, trp_size, inst):
    dataset = []
    datasetTest = []

    if inst == 0:
        inst = np.random.randint(1,20)
    ...
    print(f'Test TRP Instace (R{inst})')
    print(datasetTest)
    print('Dataset:')
    print(dataset)
...
def generate_tsplib_data(dataset_size, tsp_size):
    problem = tsplib95.load('archives/problems/tsp/st70.tsp')
    
    coord = list(problem.node_coords.values())

    max_val = -np.inf
    for c in coord:
        max_val = max(max_val, c[0], c[1])

    for i in range(len(coord)):
        coord[i] = [coord[i][0] / max_val, coord[i][1] / max_val]

    return [coord for _ in range(dataset_size)]
